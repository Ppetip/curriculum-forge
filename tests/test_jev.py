...
class ProjectGuardTests(unittest.TestCase):
    def test_semantic_review_never_edits_split(self):
        fake = Mock()
        fake.evaluate.return_value = {'answers': {'decision': {'choice': 'duplicate', 'confidence': 1}}, 'usage': {}}
        with patch.object(w.app, 'audit') as audit:
            self.assertFalse(w.run(fake)['mutates_split'])
